...
const saveCategory = () => {
    ...
    const words = editingWords.value
        .split('\n')
        .map((w) => w.trim())
        .filter((w) => w)
    if (words.length < 3) {
...
<template>
    <div class="fixed inset-0 bg-gray-900 z-50 flex flex-col overflow-hidden">
        <!-- Header -->
        <div
            class="p-4 flex items-center justify-between bg-gray-800 border-b border-gray-700 shrink-0"
        >
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <section class="space-y-4">
                <h3
                    class="text-gray-400 text-sm font-bold uppercase tracking-wider"
                >
                    Opciones de Juego
                </h3>

                <div
                    class="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 divide-y divide-gray-700"
                >
                    <label
                        class="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-750 transition-colors"
                    >
                        <div class="space-y-1">
                            <span class="text-white font-medium block"
                                >Espías se conocen</span
                            >
                            <span class="text-gray-400 text-xs block"
                                >Si hay varios espías, sabrán quiénes son sus
                                aliados.</span
                            >
                        </div>
                        <div
                    ...
                    >
                        <div class="space-y-1">
                            <span class="text-white font-medium block"
                                >Mostrar categoría al Espía</span
                            >
                            <span class="text-gray-400 text-xs block"
                                >El espía verá la categoría general, pero no el
                                lugar específico.</span
                            >
                        </div>
                        <div
                            class="relative inline-flex items-center cursor-pointer"
            ...
            <section class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <button
                        @click="openEditor()"
                        class="text-blue-400 text-sm font-medium hover:text-blue-300"
                    >
                        + Nueva
                    </button>
                </div>
                ...
                <div
                    v-if="customCategoryNames.length === 0"
                    class="text-center py-8 bg-gray-800/50 rounded-xl border border-gray-700/50 border-dashed"
                >
                ...
                <div class="space-y-2">
                    ...
                    >
                        ...
                        <div class="flex items-center space-x-2">
                            <button
                                @click="
                                    openEditor(catName, allCategories[catName])
                                "
        ...
        <div
            v-if="isEditing"
            class="absolute inset-0 bg-gray-900/95 backdrop-blur-sm z-50 flex items-center justify-center p-6"
        >
            <div
                class="bg-gray-800 w-full max-w-md rounded-2xl shadow-2xl border border-gray-700 flex flex-col max-h-full"
            >
                <div
